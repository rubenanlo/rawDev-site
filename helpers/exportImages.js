import ALTERYX from "static/assets/alteryx.svg";
import COLLAB_1 from "static/assets/collab-1.webp";
import COLLAB_2 from "static/assets/collab-2.webp";
import COLLAB_3 from "static/assets/collab-3.webp";
import COLLABORATION from "static/assets/collaboration.webp";
import CSS from "static/assets/css.svg";
import DATAVIZ_LG from "static/assets/dataviz-lg.gif";
import DATAVIZ_SM from "static/assets/dataviz-sm.webp";
import DIAPER_LG from "static/assets/diaper-lg.gif";
import DIAPER_SM from "static/assets/diaper-sm.webp";
import EMAIL from "static/assets/email.svg";
import ERROR from "static/assets/error.webp";
import EXCEL from "static/assets/excel.svg";
import EXPERTISE from "static/assets/expertise.webp";
import FINANCIAL_LG from "static/assets/financial-lg.gif";
import FINANCIAL_SM from "static/assets/financial-sm.webp";
import GIT from "static/assets/git.svg";
import GITHUB from "static/assets/github.svg";
import HTML from "static/assets/html.svg";
import INNOVATION from "static/assets/innovation.webp";
import INTEGRITY_13 from "static/assets/integrity-13.webp";
import INTEGRITY from "static/assets/integrity.webp";
import JS from "static/assets/js.svg";
import LINK from "static/assets/link.svg";
import LINKEDIN from "static/assets/linkedin.svg";
import LIVE_DEMO from "static/assets/live-demo.webp";
import LOGIN from "static/assets/login.webp";
import LOGO_ORANGE_LIGHT from "static/assets/logo-orange-light.webp";
import LOGO_ORANGE from "static/assets/logo-orange.webp";
import META_IMAGE from "static/assets/meta_image.png";
import MONGO from "static/assets/mongo.svg";
import NEXT from "static/assets/next.svg";
import NODE from "static/assets/node.svg";
import NPMJS from "static/assets/npmjs.svg";
import PHOTO_PROFILE from "static/assets/photo-profile.webp";
import PLANTHATMEAL_LG from "static/assets/planthatmeal-lg.webp";
import PLANTHATMEAL_SM from "static/assets/planthatmeal-sm.gif";
import RAWDEV_LG from "static/assets/rawdev-lg.gif";
import RAWDEV_MD from "static/assets/rawdev-md.webp";
import RAWDEV_SM from "static/assets/rawdev-sm.webp";
import REACT from "static/assets/react.svg";
import SDGTC_LG from "static/assets/sdgtc-lg.gif";
import SDGTC_SM from "static/assets/sdgtc-sm.gif";
import SHELL from "static/assets/shell.svg";
import STATIC_LG from "static/assets/static-lg.webp";
import STATIC_SM from "static/assets/static-sm.webp";
import TABLEAU from "static/assets/tableau.svg";
import TAILWIND from "static/assets/tailwind.svg";
import TRANSPARENCY_1 from "static/assets/transparency-1.webp";
import TRANSPARENCY from "static/assets/transparency.webp";
import TRASH from "static/assets/trash.svg";
import USER from "static/assets/user.svg";
import VSCODE from "static/assets/vscode.svg";

export const images = {
  ALTERYX,
  COLLAB_1,
  COLLAB_2,
  COLLAB_3,
  COLLABORATION,
  CSS,
  DATAVIZ_LG,
  DATAVIZ_SM,
  DIAPER_LG,
  DIAPER_SM,
  EMAIL,
  ERROR,
  EXCEL,
  EXPERTISE,
  FINANCIAL_LG,
  FINANCIAL_SM,
  GIT,
  GITHUB,
  HTML,
  INNOVATION,
  INTEGRITY_13,
  INTEGRITY,
  JS,
  LINK,
  LINKEDIN,
  LIVE_DEMO,
  LOGIN,
  LOGO_ORANGE_LIGHT,
  LOGO_ORANGE,
  META_IMAGE,
  MONGO,
  NEXT,
  NODE,
  NPMJS,
  PHOTO_PROFILE,
  PLANTHATMEAL_LG,
  PLANTHATMEAL_SM,
  RAWDEV_LG,
  RAWDEV_MD,
  RAWDEV_SM,
  REACT,
  SDGTC_LG,
  SDGTC_SM,
  SHELL,
  STATIC_LG,
  STATIC_SM,
  TABLEAU,
  TAILWIND,
  TRANSPARENCY_1,
  TRANSPARENCY,
  TRASH,
  USER,
  VSCODE,
};

export { ALTERYX };
export { COLLAB_1 };
export { COLLAB_2 };
export { COLLAB_3 };
export { COLLABORATION };
export { CSS };
export { DATAVIZ_LG };
export { DATAVIZ_SM };
export { DIAPER_LG };
export { DIAPER_SM };
export { EMAIL };
export { ERROR };
export { EXCEL };
export { EXPERTISE };
export { FINANCIAL_LG };
export { FINANCIAL_SM };
export { GIT };
export { GITHUB };
export { HTML };
export { INNOVATION };
export { INTEGRITY_13 };
export { INTEGRITY };
export { JS };
export { LINK };
export { LINKEDIN };
export { LIVE_DEMO };
export { LOGIN };
export { LOGO_ORANGE_LIGHT };
export { LOGO_ORANGE };
export { META_IMAGE };
export { MONGO };
export { NEXT };
export { NODE };
export { NPMJS };
export { PHOTO_PROFILE };
export { PLANTHATMEAL_LG };
export { PLANTHATMEAL_SM };
export { RAWDEV_LG };
export { RAWDEV_MD };
export { RAWDEV_SM };
export { REACT };
export { SDGTC_LG };
export { SDGTC_SM };
export { SHELL };
export { STATIC_LG };
export { STATIC_SM };
export { TABLEAU };
export { TAILWIND };
export { TRANSPARENCY_1 };
export { TRANSPARENCY };
export { TRASH };
export { USER };
export { VSCODE };
