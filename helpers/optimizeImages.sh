...
do
  cd "$dir"
  npx imagemin "**/*.{jpg,JPG,jpeg,png,webp,svg}" --plugin=mozjpeg --plugin=gifsicle --plugin=pngquant --plugin=svgo --plugin=webp --out-dir ./optimized
...
do
  ...
  if [[ $filename == *.avif ]]; then
    ...
    npx imagemin "$filename" --plugin=mozjpeg --plugin=pngquant --plugin=svgo --plugin.webp.quality=95 --out-dir ./optimized
